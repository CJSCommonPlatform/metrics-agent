...
public class ArtemisAgent {
    ...
    private static void addTimerStartingCode(final ClassPool classPool, final CtClass queueClass) throws NotFoundException, CannotCompileException {
        ...
        addTailMethod.insertBefore("uk.gov.justice.metrics.timer.context.JmsDestinationTimerContextFactory.timerContextOf(name.toString(), ref.getMessage().getAddress().toString()).startTimer(String.valueOf(ref.getMessage().getMessageID()));");
    }
    ...
    private static void addTimerStoppingCode(final ClassPool classPool, final CtClass queueClass) throws NotFoundException, CannotCompileException {
        ...
        proceedDeliver.insertAfter("uk.gov.justice.metrics.timer.context.JmsDestinationTimerContextFactory.timerContextOf(name.toString(), reference.getMessage().getAddress().toString()).stopTimer(String.valueOf(reference.getMessage().getMessageID()));");
    }
